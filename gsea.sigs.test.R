...
test.matrix2 <- convert.sigs.to.matrix(test.sigs, genes)
# make the comparison and get the odds ratios and p.values
test.compare <- compare.sigs(test.matrix, test.matrix2)

# if you are running on a Unix machine with multiple cores you can do this in
# parallel
library(parallel)
options(mc.cores = 8)
system.time(test.compare <- compare.sigs(test.matrix, test.matrix2))

sig.matrix <- convert.sigs.to.matrix(sigs, genes)
test.matrix <- convert.sigs.to.matrix(test.sigs, genes)

# make the comparison and get the odds ratios and p.values
system.time(test.compare <- compare.sigs(sig.matrix, test.matrix))
